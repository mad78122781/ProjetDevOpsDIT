...
pipeline{
    ...
    environment {
        ...
        VERSION_IMAGE_APP = '0.0.1-SNAPSHOT'
         repo = "${JOB_NAME}"
         DOCKER_REGISTRY_USER= '128.7.8.2'
         DOCKER_REGISTRY_USER_PASSWORD= 'root'
         SONARQUBE_URL= '128.7.8.2'
            ...
            steps{
                ...
                nexusArtifactUploader artifacts: [
                    [
                        artifactId: 'auth',
                        classifier: '',
                        file: 'target/auth-0.0.1-SNAPSHOT.jar',
                        type: 'jar'
                    ]
                ],
                credentialsId: 'nexus',
                ...
                version: '0.0.1-SNAPSHOT'
                    ...
                    stage('Build Image'){
                          steps{
                            script {
                             sh "docker build -t ${DOCKER_REGISTRY_URL}/${env.APP_NAME} ."
                            }
                          }
                        }
                    ...
                    stage ('Push Docker Image'){
                        steps{
                            script{
                          sh "docker push ${DOCKER_REGISTRY_URL}/${env.APP_NAME}:latest" 
                        ...
                        steps{
                        sh "docker rmi ${DOCKER_REGISTRY_URL}/${env.APP_NAME}:latest"
                    }
                    }
             ...
             stage('Deploy to cluster') {
                  steps {
                    script {
                       sshagent(['aopdeploy']) {
                           sh'ssh -oStrictHostKeyChecking=no root@10.0.0.20 kubectl delete deploy --ignore-not-found=true auth-app'
                                                      sh'ssh -oStrictHostKeyChecking=no root@10.0.0.20 kubectl delete svc --ignore-not-found=true aop-svc'
                                     sh 'ssh -oStrictHostKeyChecking=no root@10.0.0.20 kubectl apply -f /var/lib/jenkins/workspace/auth_main@2/deploymentservice.yml'
                                     sh 'sleep 30'
                                }
